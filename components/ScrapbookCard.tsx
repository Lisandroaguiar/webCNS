...
const accentClasses = {
  lime: "bg-lime",
  fuchsia: "bg-cronopios-paper text-ink",
  cyan: "bg-cyan"
} as const;
...
export function ScrapbookCard({
  ...
  return (
    <article className={`relative min-w-0 border-2 border-black p-5 pt-7 shadow-brutal transition-transform hover:-translate-y-1 ${accentClasses[accent]}`}>
      ...
      {linkUrl && <a href={linkUrl} target="_blank" rel="noreferrer" className="button-primary mt-5 text-sm">Ver actividad ↗</a>}
    </article>
  );
}
