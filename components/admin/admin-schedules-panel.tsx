...
export function AdminSchedulesPanel({ initialSchedules, subjects }: { initialSchedules: Row[]; subjects: Subject[] }) {
  ...
  const [editorAnchor, setEditorAnchor] = useState<number | null>(null);
  const [draft, setDraft] = useState<Draft>(blank);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const [messageAnchor, setMessageAnchor] = useState<number | null>(null);
  function edit(row: Row) { setEditing(row.id); setEditorAnchor(row.id); setMessage(""); setDraft({ ...row, start_time: row.start_time?.slice(0, 5) ?? "", end_time: row.end_time?.slice(0, 5) ?? "" }); }
  function startNew(row?: Row) { setEditing("new"); setEditorAnchor(row?.id ?? null); setMessage(""); setDraft(row ? { ...row, commission: "", start_time: row.start_time?.slice(0, 5) ?? "", end_time: row.end_time?.slice(0, 5) ?? "" } : blank()); }
  async function save(publish = false) {
    setBusy(true); setMessage(""); setMessageAnchor(editorAnchor);
    try {
  ...
  const editor = editing !== null && <section className={editorAnchor === null ? "card mt-5" : "mt-5 border-t-2 border-ink/15 pt-5"}><h2 className="font-display text-xl font-black">{editing === "new" ? "Nueva comisión" : "Editar horario"}</h2><div className={`mt-4 grid gap-3 ${editorAnchor === null ? "sm:grid-cols-2 lg:grid-cols-3" : "xl:grid-cols-2"}`}>
  ...
  return <div className="mt-6">
    {message && messageAnchor === null && <p role="status" className="mb-4 border-l-4 border-cronopios-magenta bg-white p-3 text-sm font-bold">{message}</p>}
    ...
    {editing !== null && editorAnchor === null && editor}
    <p className="mt-5 text-sm text-ink/60">{visible.length} de {rows.length} horarios disponibles para editar</p><div className="mt-3 grid gap-3 md:grid-cols-2">{visible.slice(0, displayLimit).map(row => <article key={row.id} className="card"><p className="font-mono text-[10px] font-bold uppercase tracking-wide text-ink/60">{row.academic_year} · {row.status === "published" ? "Publicado" : row.status === "draft" ? "Borrador" : row.status === "verified" ? "Verificado" : "Obsoleto"} · {row.source_label}</p><h2 className="mt-2 font-display text-lg font-black">{row.raw_subject_name}</h2><p className="text-sm text-ink/60">{row.commission || "Sin comisión"} · {row.weekday} {row.start_time?.slice(0, 5)}{row.end_time ? `–${row.end_time.slice(0, 5)}` : ""} · {row.classroom || "Aula a confirmar"}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => edit(row)} className="min-h-11 border-2 border-ink px-3 font-bold">Editar</button><button type="button" onClick={() => startNew(row)} className="min-h-11 border-2 border-ink px-3 font-bold">Otra comisión</button></div>{message && messageAnchor === row.id && <p role="status" className="mt-4 border-l-4 border-cronopios-magenta bg-white p-3 text-sm font-bold">{message}</p>}{editing !== null && editorAnchor === row.id && editor}</article>)}</div>{visible.length > displayLimit && <button type="button" className="mt-5 min-h-11 border-2 border-ink bg-white px-4 font-bold" onClick={() => setDisplayLimit(current => current + 80)}>Mostrar más horarios</button>}
  </div>;
}
