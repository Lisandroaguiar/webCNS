...
export default async function AdminImportsPage() {
  ...
  const [{ data: sources }, { data: runs }, { data: eventDrafts }, { data: scheduleDrafts }] = await Promise.all([
    admin.from("data_sources").select("id,key,name,last_checked_at,last_success_at").neq("key", "admin-manual-schedules").order("name"),
  ...
  return <main className="min-h-screen bg-cronopios-paper px-5 py-8 md:px-8"><div className="mx-auto max-w-6xl"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Administración</p><h1 className="mt-2 font-display text-4xl font-black">Imports FDA</h1></div><div className="flex gap-4"><Link href="/admin/horarios" className="font-bold underline">Editar horarios</Link><Link href="/admin/cartelera" className="font-bold underline">Administrar cartelera</Link></div></div><p className="mt-3 max-w-2xl text-ink/65">Las importaciones se guardan como draft hasta una revisión humana.</p><AdminImportsPanel sources={sources ?? Object.values(sourceRegistry).map(source => ({ id: source.key, key: source.key, name: source.name, last_checked_at: null, last_success_at: null }))} runs={runs ?? []} drafts={[...(eventDrafts ?? []).map(item => ({ ...item, table: "academic_events" as const, label: item.title })), ...(scheduleDrafts ?? []).map(item => ({ ...item, table: "course_schedules" as const, label: `${item.raw_subject_name} · ${item.weekday} ${formatScheduleTime(item.start_time)}` }))]} /></div></main>;
}
