...
function fields(body: Record<string, unknown>, allowUnspecifiedPlan = false) {
  const raw_subject_name = String(body.raw_subject_name ?? "").trim();
  const curriculum = body.curriculum == null || body.curriculum === "" ? null : String(body.curriculum);
  ...
  if (!raw_subject_name || raw_subject_name.length > 160 || (!allowUnspecifiedPlan && !curriculum) ||
    (curriculum !== null && !["new", "old", "profesorado"].includes(curriculum)) ||
...
export async function PATCH(request: Request) {
  ...
  if (!existing) return NextResponse.json({ error: "El horario ya no existe." }, { status: 404 });
  const publish = body.publish === true;
  const { error } = await admin.from("course_schedules").update({ ...values, subject_id: body.subject_id ? String(body.subject_id) : null, status: publish ? "published" : existing.status, updated_at: new Date().toISOString() }).eq("id", id);
