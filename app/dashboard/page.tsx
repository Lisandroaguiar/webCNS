...
import { FdaHomeHero, fdaVariants } from "@/components/identity/fda-identity";
...
export default async function DashboardPage() {
  ...
  return <div className="min-w-0 space-y-5 sm:space-y-6">
    <FdaHomeHero name={displayName} variant={fdaVariants[Number(today.slice(-2)) % fdaVariants.length]} />
    ...
    <section className="card bg-cronopios-paper" aria-labelledby="dashboard-agenda"><p className="eyebrow">Próximo</p><h2 id="dashboard-agenda" className="font-display text-xl font-black">Mi agenda</h2>{nextItems.length ? <ul className="mt-2 divide-y divide-ink/15">{nextItems.map(item => <li key={item.key} className="flex min-w-0 flex-wrap gap-x-3 py-2 text-sm"><span className="font-mono font-bold">{item.date === today ? "Hoy" : formatDate(item.date)}{item.start_time ? ` · ${item.start_time.slice(0, 5)}` : ""}</span><span className="min-w-0 font-semibold [overflow-wrap:anywhere]">{item.title}</span><span className="text-xs font-bold uppercase text-ink/55">{item.kind === "course" ? "Cursada" : item.kind === "fda" ? "FDA" : item.category ?? "Personal"}</span></li>)}</ul> : <p className="mt-2 text-sm text-ink/60">Tu agenda todavía está vacía.</p>}<Link href="/dashboard/agenda" className="mt-2 inline-flex min-h-11 items-center gap-1 font-bold text-cronopios-magenta underline">Ver mi agenda <ArrowRight size={16} /></Link></section>
